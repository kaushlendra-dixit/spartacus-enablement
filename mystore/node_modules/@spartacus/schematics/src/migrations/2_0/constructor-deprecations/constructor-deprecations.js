    ...
    return (tree, context) => {
        ...
        for (const originalSource of sourceFiles) {
            ...
            for (const constructorDeprecation of constructor_deprecation_data_1.CONSTRUCTOR_DEPRECATION_DATA) {
                if (!file_utils_1.isCandidateForConstructorDeprecation(originalSource, constructorDeprecation)) {
                    continue;
                }
                for (const newConstructorParam of constructorDeprecation.addParams ||
                    ...
                    const changes = file_utils_1.addConstructorParam(source, sourcePath, constructorNode, newConstructorParam);
                ...
                for (const constructorParamToRemove of constructorDeprecation.removeParams ||
                    ...
                    const nodes = ast_utils_1.getSourceNodes(source);
                    const constructorNode = file_utils_1.findConstructor(nodes);
                    const changes = file_utils_1.removeConstructorParam(source, sourcePath, constructorNode, constructorParamToRemove);
                    file_utils_1.commitChanges(tree, sourcePath, changes, file_utils_1.InsertDirection.RIGHT);
                }
            }
        }
