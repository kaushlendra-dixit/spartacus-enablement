...
function getSuperNode(constructorNode) {
    const superNodes = ast_utils_1.findNodes(constructorNode, ts.SyntaxKind.SuperKeyword);
...
function getParams(constructorNode, camelizedParamNames) {
    ...
    const params = ast_utils_1.findNodes(callExpressions[0], ts.SyntaxKind.Identifier);
...
function updatePackageJson(appTree, filePath, type, pkg, version) {
    const packageContent = appTree.read(core_1.normalize(filePath));
    if (!packageContent) {
        return;
    }
